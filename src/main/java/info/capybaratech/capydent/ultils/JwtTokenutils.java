...
public class JwtTokenutils {
  ...
  public  String generateTokenForIdentity(String identity) {
        ...
        return JWT.create()
                .withIssuer("SmileServer")
                .withAudience("SmileClient")
                .withExpiresAt(Instant.now().plus(jwtTokenExpiration, ChronoUnit.MINUTES))
                .withClaim("identity", identity)
                .withIssuedAt(Instant.now())
                .sign(alg);
    }

    public String validateAndGetIdentity(String token) {
        var alg = Algorithm.HMAC256(jwtSecret);
    ...
    public String generateNewTokenFromRefresh(String oldToken) {
        var identity = validateAndGetIdentity(oldToken);
        return generateTokenForIdentity(identity);
    }

    public String generateNewRefreshTokenFromRefresh(String oldToken) {
        var identity = validateAndGetIdentity(oldToken);
    ...
    public String generateRefreshTokenForIdentity(String identity) {
        ...
        return JWT.create()
                .withIssuer("SmileServer")
                .withAudience("SmileClient")
                .withExpiresAt(Instant.now().plus(jwtRefreshTokenExpiration, ChronoUnit.MINUTES))
                .withClaim("identity", identity)
                .withIssuedAt(Instant.now())
                .sign(alg);
    }
